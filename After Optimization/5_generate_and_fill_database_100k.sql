...
--  _______  _______           _______  _______  _______      __    _______  _______    _       
...
-- \_______)(_______/|/     \|(_______/|/     \||/     \|    \____/(_______)(_______)  |_/    \/
                                                                                             


-- clear the whole schema
DROP SCHEMA IF EXISTS optimized_100k CASCADE;
CREATE SCHEMA optimized_100k;

GRANT ALL ON SCHEMA optimized_100k TO bob;
GRANT ALL ON SCHEMA optimized_100k TO public;
...
CREATE TABLE IF NOT EXISTS optimized_100k.students
(
    id int ,
    name text  NOT NULL,
...
insert into optimized_100k.students(id , "name" , username , "password" )
select id,
...
CREATE TABLE IF NOT EXISTS optimized_100k.instructors
(
    id int ,
    name text  NOT NULL,
    username text unique NOT NULL,
...
insert into optimized_100k.instructors(id , "name" , username , "password" )
select id,
...
CREATE TABLE IF NOT EXISTS optimized_100k.courses
(
    id int ,
    name text unique  NOT NULL,
    instructor_id int NOT NULL REFERENCES optimized_100k.instructors(id),
    cost integer NOT NULL,
    rate integer NOT NULL,
    CONSTRAINT courses_pkey PRIMARY KEY (id)
);

insert into optimized_100k.courses(id , "name" , instructor_id , cost , rate )
select id,
       concat ('course ' , id ), 
       floor(1 + random()*1000), 
...
CREATE TABLE IF NOT EXISTS optimized_100k.videos
(
    id int ,
    name text NOT NULL,
    ...
    course_id int NOT NULL REFERENCES optimized_100k.courses(id),
    CONSTRAINT videos_pkey PRIMARY KEY (id)
);

insert into optimized_100k.videos(id , "name" , instructor_name , course_name , link , course_id )
select id,
       concat ('video ' , floor(1 + random()*10000)), 
       concat ('instructor ' , floor(1 + random()*1000)), 
...
CREATE TABLE IF NOT EXISTS optimized_100k.students_courses
(
    student_id int NOT NULL REFERENCES optimized_100k.students(id),
    course_id int NOT NULL REFERENCES optimized_100k.courses(id),
    course_name text NOT NULL,
...
insert into optimized_100k.students_courses(student_id , course_id , course_name , student_name )
select floor(1 + random()*100000), 
       floor(1 + random()*10000), 
       concat ('course ' , floor(1 + random()*1000)), 
       concat ('student ' , floor(1 + random()*1000))
...
------------------------- update student_name ----------------------------
update optimized_100k.students_courses sc
set student_name = s.name
from optimized_100k.students s
where sc.student_id = s.id;
------------------------- update course_name ----------------------------
update optimized_100k.students_courses 
...
CREATE TABLE IF NOT EXISTS optimized_100k.quizzes
(
    id int ,
    name text NOT NULL,
    course_id int NOT NULL REFERENCES optimized_100k.courses(id),
...
insert into optimized_100k.quizzes(id , "name" , course_id )
select id,
       concat ('quiz ' , floor(1 + random()*10000)), 
       floor(1 + random()*10000)
       from generate_series(1, 100000) as id 
...
CREATE TABLE IF NOT EXISTS optimized_100k.students_videos
(
    student_id int NOT NULL REFERENCES optimized_100k.students(id),
    video_id int NOT NULL REFERENCES optimized_100k.videos(id),
...
with students_courses_videos as (
    ...
    from optimized_100k.students_courses sc
    join optimized_100k.videos v on v.course_id = sc.course_id
)

insert into optimized_100k.students_videos(student_id , video_id)
...
CREATE TABLE IF NOT EXISTS optimized_100k.students_quizzes
(
    student_id int NOT NULL REFERENCES optimized_100k.students(id),
    student_name text NOT NULL,
    quiz_id int NOT NULL REFERENCES optimized_100k.quizzes(id),
...
with students_courses_quizzes as (
    ...
    from optimized_100k.students_courses sc
    join optimized_100k.quizzes q on q.course_id = sc.course_id
)

insert into optimized_100k.students_quizzes(student_id , student_name, quiz_id , score)
    ...
    join optimized_100k.students s on s.id = students_courses_quizzes.student_id
    order by random()
    limit 1000000
